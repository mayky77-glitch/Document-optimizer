...
def test_summary_has_eight_rows_per_index_all_indices_formulas_and_calc_flags(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        assert tuple(summary.cell(1, column).value for column in range(1, 6)) == SUMMARY_HEADERS
        assert summary.max_row == 1 + len(CATEGORY_ORDER) * (len(layouts) + 1)
        assert [summary.cell(row, 1).value for row in range(2, 10)] == ["1001"] * 8
        assert [summary.cell(row, 1).value for row in range(26, 34)] == ["Все индексы"] * 8
        assert summary["D2"].value.startswith("=SUMIF(")
        assert summary["E2"].value.startswith("=SUMIF(")
        assert summary["D26"].value.startswith("=SUMIF(")
        assert summary["D2"].number_format == FRACTIONAL_QUANTITY_FORMAT
        assert summary["E2"].number_format == COST_FORMAT
