...
def test_normalizes_every_row_without_losing_source_provenance_or_decimals(
    make_training_row,
) -> None:
...
def test_preserves_leading_zeros_unicode_units_and_stable_tokens(make_training_row) -> None:
    source = make_training_row()
...
def test_typos_are_replaced_only_by_exact_normalized_dictionary_key(make_training_row) -> None:
...
def test_same_business_key_has_same_line_id_across_physical_sources_and_warns(
    make_training_row,
) -> None:
...
def test_different_business_keys_get_different_ids_and_result_is_deterministic(
    make_training_row,
) -> None:
    first = make_training_row()
    second = replace(
