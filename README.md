...
Интегрированы **блоки 1–6 — от инвентаризации и выбора источника до схемы книги
и извлечения канонических строк** (текущая версия пакета `0.6.0`).
...
сохраняет формулы и provenance ячеек и атомарно записывает JSONL вместе с
`*.meta.json`. Исходный Excel не изменяется.

```bash
report-processor extract-rows \
  --selection output/source_selection.json \
  --schema output/workbook_schema.json \
  --output output/extracted_rows.jsonl
```
...
листа и типа, лимиты и форматы JSON/JSONL. Неразрешённые или не-OK столбцы
...
## Ограничения блоков 1–6

Намеренно не реализованы:

- бизнес-классификация, сопоставление, расчёты и изменение Excel;
- удаление итогов, разделов и дублей из извлечённых исходных строк;
- сопоставление документов и работ;
- расчёты количества и стоимости;
- DuckDB, Parquet и pandas;
...
Потоковый JSONL и `*.meta.json` записываются атомарно и допускают восстановление;
JSON-вывод использует тот же безопасный подход.
