...
Страница **«Отчёт (карточка остатков)»** создаёт или обновляет отчёт.
...
- **Обновить существующую** — загрузить те же источники и один готовый отчёт
  `.xlsx`.
...
совместимый запасной путь. Внутренний файл остаётся `drawing-card.xlsx`, а
скачиваемый отчёт получает имя `Отчёт по остаткам за <месяц> <год>.xlsx` или
`Отчёт по остаткам.xlsx`, если период не выбран. ZIP, каталоги, неверные
сигнатуры Excel, небезопасные имена и превышение лимита отклоняются. Этот процесс
принимает только `.xlsx`, `.xlsm` и `.xlsb`; LibreOffice Calc `.ods` и PDF
доступны только в отдельном сравнении Excel с PDF-отчётами, где PDF необязателен.
