...
### Отдельная функция «Карточка остатков»

В верхнем меню панели выберите **«Карточка остатков»** или откройте
...
Период необязателен. Если автоматической уверенности недостаточно, панель
предлагает скачать `manual_review.xlsx`, заполнить контролируемые решения и
загрузить файл обратно. Готовый результат скачивается как `drawing-card.xlsx`.
ZIP, каталоги, неверные сигнатуры Excel, небезопасные имена и превышение лимита
отклоняются.

Файлы обрабатываются только в приватном локальном workspace. В JSON и заголовки
не попадают серверные пути; RAG и сетевые модели для этой функции выключены.
Исходное проверенное имя сохраняется только у приватной копии, поскольку код
объекта может быть частью имени. Оригиналы остаются неизменными.

Локальный acceptance: full real+slow+RAG **624 passed, 2 skipped in 100.67s**;
отдельный real demo — **1 passed** с **7** извлечёнными строками, **32** строками
карточки и **0** строками manual review. Desktop/mobile browser, keyboard,
create/download, wheel `[rag]`, bundled resources, Ruff, format, compileall,
JS syntax и diff-check — PASS; console/page/external-request errors — **0**.
