...
def default_rules_path() -> Path:
    return project_root() / "config" / "drawing_card" / "rules.json"


def default_examples_path() -> Path:
    return project_root() / "config" / "drawing_card" / "confirmed_examples.jsonl"


def default_template_path() -> Path:
    return project_root() / "templates" / "default_drawing_card_template.xlsx"


def _validate_request(request: WorkflowRequest) -> None:
    source_count = (
        int(bool(request.inputs))
        + int(request.input_dir is not None)
        + int(request.archive is not None)
    )
    if source_count != 1:
        raise ValueError("Specify exactly one of --inputs, --input-dir or --archive")
    ...
    if request.output is not None and not request.dry_run:
        ...
        input_containers = [*request.inputs]
        if request.archive is not None:
            input_containers.append(request.archive)
        if any(output_path == path.expanduser().resolve() for path in input_containers):
            raise ValueError("--output must not overwrite an input file or archive")
        if request.input_dir is not None:
            input_dir = request.input_dir.expanduser().resolve()
            if output_path.is_relative_to(input_dir):
                raise ValueError("--output must not be placed inside --input-dir")
    ...
    if request.input_dir is not None and not request.input_dir.is_dir():
        raise NotADirectoryError(request.input_dir)
    if request.archive is not None and not request.archive.is_file():
        raise FileNotFoundError(request.archive)


def _container_paths(request: WorkflowRequest) -> list[Path]:
    paths = list(request.inputs)
    if request.archive is not None:
        paths.append(request.archive)
    if request.input_dir is not None:
        allowed = {".xlsx", ".xlsm", ".xlsb", ".zip"}
        paths.extend(
            path
...
def run_workflow(request: WorkflowRequest) -> WorkflowResult:
    ...
    manifest = build_manifest(request.inputs, request.input_dir, request.archive)
