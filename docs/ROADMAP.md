...
Owner must approve: Table 1 sheet/whole-period semantics; M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; stage/version selection; month/current-period semantics; quantity destination and new-column meaning; coefficient 2.7 basis; J/L equality/tolerance; behavior when a process exists only under another unit, unit conversion and multiple-source-unit display; formula freshness/recalc; precision/overwrite; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Semantic unit comparison, red `old/source`, quantity from matching-unit rows, cost from all approved rows, `0/0` for no process-name candidates and checkbox review are already fixed. M02/M06 literals are already fixed. Exit: signed decision record and fixtures updated. No implementation fallback is allowed.
...
| P5 review surfaces / P2,P3,P4 | developer P3; tester P3; designer P4 read-only spec | developer: `src/review/`, `src/cli/`, `src/site/api/`, `src/site/state/`, `src/site/components/`, `src/site/styles/`, `src/model_gateway/`; tester: `tests/review/`, `tests/model_gateway/`; designer: no write scope | Exactly two uploads, CLI args, month/stage UI, one-table candidate checkboxes/comments/live totals and model modes disabled/CLI/manual-app/optional-API. Designer supplies read-only visual spec because templating/site code belongs to developer; no shared ownership. | Candidate API stable → `0/0` no-match, recommended/uncertain preselection, checkbox/comment/live-recalculation, labels/count, report XLSX/source folder/ZIP, month/stage preservation, manual no-GPT, CLI/manual-app schema round-trip, token and export-block tests pass. |
| P6 export + verify / P3,P5 | developer P4; tester P3 | developer: `src/export/`, `src/verify/`; tester: `tests/export/` | New editable copy, atomic save/reopen/manifest. No source overwrite. | Review resolution available → preservation/formula/style/merged/filter/comment/color/original-hash/manifest tests pass. |
...
1006 goldens above; schema fixtures 1004/0919/KITSO; exact upload labels/count, report XLSX, source folder/ZIP, stage/month preservation, misleading filename/conflicting content, candidate sets/missing 1005/0768/0778, M04/M05 positive/cross-category/exclusion, ambiguous mapping, unit mismatch, stale formula, duplicate file/row, M13/M14 collision, feedback drift/rollback and order independence. GPT tests prove strict rejection and no authority. Every package records commands, result, risks and owner approval before exit.
