...
Owner must approve: feedback retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. User-feedback-only exact-scope memory with default-on direct switch and post-export activation, M04/M05 flow and prior rules are fixed. M02/M06 literals are fixed.
...
| P4 feedback memory / P3 | developer P4; tester P3; database-engineer P4 read-only consult | developer: `src/feedback/`, `src/storage/`; tester: `tests/feedback/`; database-engineer: no write scope | Compact exact-scope user-feedback rules, default-on switch events, post-export activation, active snapshot + audit. No auto-match memory or category generalization. | Confident-auto creates none; changed/uncertain on/off, cancelled/failed/successful export, exact reuse, drift/conflict, dedup/storage/latency/compaction tests pass. |
