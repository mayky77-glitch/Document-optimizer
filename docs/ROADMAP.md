...
Owner must approve: Table 1 sheet/whole-period semantics; M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; stage/version selection; month/current-period semantics; quantity destination and new-column meaning; coefficient 2.7 basis; J/L equality/tolerance; no-compatible-quantity behavior, unit conversion and multiple-source-unit display; formula freshness/recalc; precision/overwrite; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Semantic Table-2/Table-1 unit comparison, observed at F/J but movable, red `old/source`, quantity from matching-unit rows and cost from all approved rows are already fixed. M02/M06 literals are already fixed. Exit: signed decision record and fixtures updated. No implementation fallback is allowed.
...
| P5 review surfaces / P2,P3,P4 | developer P3; tester P3; designer P4 read-only spec | developer: `src/review/`, `src/cli/`, `src/site/api/`, `src/site/state/`, `src/site/components/`, `src/site/styles/`, `src/model_gateway/`; tester: `tests/review/`, `tests/model_gateway/`; designer: no write scope | Exactly two uploads, CLI args, month/stage UI, one-table review and model modes disabled/CLI/manual-app/optional-API. Designer supplies read-only visual spec because templating/site code belongs to developer; no shared ownership. | Candidate API stable → labels/count, report XLSX/source folder/ZIP, month/stage preservation, manual no-GPT, CLI/manual-app schema round-trip, no-AI-on-deterministic-result, token ceiling/no-full-history, decision and export-block tests pass. |
