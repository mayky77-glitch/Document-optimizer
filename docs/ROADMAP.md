...
Owner must approve: M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Default-off explicit-only conversion, semantic KS-6a scope, file ranking, standalone output and prior rules are fixed. M02/M06 literals are already fixed.
...
| P3 baseline rules + calculation / P1,P2 | developer P4; tester P3 | developer: `src/rules/`, `src/calc/`; tester: `tests/rules/`, `tests/calc/` | M01–M14 registry, Decimal/lineage and default-off explicit conversion registry. No feedback/UI/export. | Includes/excludes/suffix/collision, no-implicit-conversion, exact approved pair/factor, rollback, Decimal/order/golden/unit/J-L tests pass. |
