...
Owner must approve: M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; automatic unit conversion; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Semantic KS-6a/whole-period selection with explicit ambiguity handling, file ranking, missing-cache recovery, standalone output and prior rules are fixed. M02/M06 literals are already fixed.
