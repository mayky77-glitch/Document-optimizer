...
Owner must approve: Table 1 sheet/whole-period semantics; M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; stage/version selection; month/current-period semantic validation; automatic unit conversion; formula freshness/recalc; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Month-pair create/reuse/nonblank-overwrite flow, full-precision/two-decimal values, J/L comparison, coefficient flow, unit/cost rules, `0/0` and checkbox review are fixed. M02/M06 literals are already fixed. Exit: signed decision record and fixtures updated. No implementation fallback is allowed.
...
| P6 export + verify / P3,P5 | developer P4; tester P3 | developer: `src/export/`, `src/verify/`; tester: `tests/export/` | Semantic month-pair reuse/create, old→new confirmation, new editable copy, atomic save/reopen/manifest. No source overwrite or duplicate month pair. | Review resolution available → absent/existing/duplicate pair, styled right append, blank/nonblank destination, preservation/formula/style/merged/filter/comment/color/original-hash/manifest tests pass. |
