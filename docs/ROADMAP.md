...
Owner must approve: M04 power include/supporting-work policy; M05 low-current include policy; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Exact/normalized-prefix suffix semantics, M14/M13 ownership and prior rules are fixed. M02/M06 literals are fixed.
...
| P3 baseline rules + calculation / P1,P2 | developer P4; tester P3 | developer: `src/rules/`, `src/calc/`; tester: `tests/rules/`, `tests/calc/` | M01–M14 registry, prefix matcher, exclusive ownership, Decimal/lineage and conversion registry. | Exact/base+diameter/mark/number/text prefix positives, non-prefix negatives, hard-exclude precedence, M14/M13/no-double-count and calculation tests pass. |
