...
Owner must approve: Table 1 sheet/whole-period semantics; M04 exact power include set and supporting works; M05 exact low-current include set; M13/M14 precedence; M03/M07/M08/M12 suffix semantics; stage/version selection; month/current-period semantics; quantity destination and new-column meaning; J/L blank behavior and two-decimal rounding mode; automatic unit conversion; formula freshness/recalc; output precision outside J/L/overwrite; feedback reuse compatibility, retention, deactivate/rollback and user authority; AI context/token budget and storage/retrieval thresholds. Numeric J/L comparison after two-decimal rounding, run coefficient/warning flow, semantic unit rules, cost aggregation, `0/0` no-match and checkbox review are fixed. M02/M06 literals are already fixed. Exit: signed decision record and fixtures updated. No implementation fallback is allowed.
