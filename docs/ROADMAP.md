...
Owner must approve: AI context/token budget and storage/retrieval thresholds. User-feedback-only exact memory with indefinite version retention, compact on/off/restore UI, post-export activation and no physical deletion is fixed. M02/M06 literals are fixed.
