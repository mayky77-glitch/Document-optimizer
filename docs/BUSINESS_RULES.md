...
`0` файлов-кандидатов для индекса — не нулевое выполнение. Весь Table-2 object block этого индекса (строка с непустым `B` и связанные строки до следующего object block) получает status `source_missing_carry_forward_pending` и не участвует в Table-1 matching, unit или coefficient checks. Site показывает одну прямую object-level развилку без modal: **«Перенести прошлый период»** (recommended) / **«Оставить новый период пустым»**. До выбора export ждёт решения, но состояние не показывается как ошибка.

При переносе скрипт семантически находит непосредственно предыдущую month quantity/cost pair Table 2 и построчно копирует в выбранную новую pair только сохранённые значения: valid numeric → numeric, blank → blank; формулы не переносятся. J/new-quantity comparison и yellow rule выполняются уже по скопированному значению, а unit/coefficient checks помечаются `not_recalculated_carried_forward`. Если destination nonblank, тот же экран показывает `старое → переносимое`, и object-level confirm одновременно является overwrite confirmation. Невалидное прошлое plain value показывает cell evidence; пользователь может исправить файл либо выбрать «Оставить новый период пустым». Formula без cached value остаётся общим Table-2 flattening blocker и не обходится blank branch. Оба решения и все затронутые координаты входят в audit/manifest. Для stage 13.1 такой выбор нужен для 1005, 0768 и 0778. Если файл найден, но в нём нет semantic KS-6а/whole-period block, это schema blocker, а не missing-index carry-forward. Неразрешённая множественность файлов также остаётся blocker до user confirmation. Тесты включают leading zero, boundary, `6а`/`6a`/case, `~$`, semantic stage/month, misleading filename, `ред1/ред2/no-revision`, missing-index carry/blank branches and no-formula copy, misleading mtime, schema-quality tie и explicit confirmation.
