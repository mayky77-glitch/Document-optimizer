...
- `Table2.J` и `Table2.L` сравниваются как числовые `Decimal`, не как текст. Оба значения округляются до двух знаков только для этого сравнения; после округления точное равенство подсвечивается жёлтым без дополнительного tolerance. Форматы `1`, `1.0`, `1.000` равны; `0 = 0` также жёлтый. Две пустые ячейки считаются неизменившимися и тоже жёлтые; если пустая только одна, равенства и жёлтой подсветки нет. Пустое значение никогда автоматически не превращается в числовой ноль. Rounding mode остаётся Gate 0.
