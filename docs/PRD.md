...
No scaffold or implementation starts until owner approves every item in BUSINESS_RULES §7: M04/M05 exact include sets, M13/M14, suffix/supporting-work semantics, period semantics, quantity/new columns, 2.7 basis, J/L tolerance, automatic unit conversion, versions/stage, month/current-period semantics, freshness, display/overwrite, feedback reuse/retention/rollback and AI context/token budget plus performance/storage thresholds. Unit-priority, alternative-unit grouping and all-approved-row cost inclusion are fixed. M02/M06 literals are already fixed. CodeGraph only follows first scaffold.
