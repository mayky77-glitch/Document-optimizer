...
No scaffold or implementation starts until owner approves every item in BUSINESS_RULES §7: M04/M05 exact include sets, M13/M14, suffix/supporting-work semantics, period semantics, quantity/new columns, J/L blank behavior and rounding mode, automatic unit conversion, versions/stage, month/current-period semantics, freshness, other output precision/overwrite, feedback reuse/retention/rollback and AI context/token budget plus performance/storage thresholds. Numeric two-decimal J/L comparison, coefficient flow and unit/cost rules are fixed. M02/M06 literals are already fixed. CodeGraph only follows first scaffold.
