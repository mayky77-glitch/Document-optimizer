...
- Selection uses suffix-after-last-dot, Unicode token boundaries and `6а` Cyrillic/Latin/case; semantic workbook content, not filename, proves stage. Missing index source creates one whole-object carry-previous-period/leave-blank choice; multiple files still require review.
...
- Missing Table-1 file and no process match are distinct: missing index source requires object-level carry of the previous semantic Table-2 month pair or explicit blank; `0/0` applies only when a valid source file exists but contains no process-name candidate.
