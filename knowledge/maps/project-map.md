...
- В сверке документов пассивные замечания показываются отдельно от нерешённых
  ...
  карточки. Карточки ручной проверки и подсказок используют общий компактный
  каркас `review-item`: заголовок со статусом, ячейки контекста и нижняя строка
  решения. Ручная проверка показывает причину и количество, её нижняя строка —
  охват группы, действие и ровно `Одобрить`/`Отклонить`; подсказка сохраняет
  `fit`/`not_fit` и показывает кандидата, цель и оценку. Одинаковые пассивные
  предупреждения также сжимаются в одну строку с числом повторений, сохраняя
  различающиеся поля иерархии. Выгрузка доступна лишь после закрытия обоих
  наборов решений.
