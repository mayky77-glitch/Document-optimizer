...
- Форматы: целое количество `0`, дробное `0.###`, стоимость `#,##0.000`.
- XLSX содержит лист `Сводный отчет`: восемь категорий для каждого индекса и
  восемь итоговых строк `Все индексы`. Стоимость суммируется формулами всегда;
  количество — только при полной совместимости непустых единиц измерения.
- Незанятые правые секции шаблона удаляются, поэтому пустой четвертый макет не
  отображается при трех индексах.
- Экран карточки имеет прямой переключатель светлой/темной темы. В группе
  проверки используется один ряд: категория, режим учета, `Применить` и
  `Отклонить`; отдельные дублирующие кнопки смены категории запрещены.
...
- Реальный артефакт: 3 индекса, 972 чертежа, 24 индексные и 8 общих строк,
  64 сохраненные формулы; Decimal-сверка с точностью `0.001`.
