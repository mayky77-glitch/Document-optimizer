...
- В сверке документов пассивные замечания показываются отдельно от нерешённых
  решений. Повторяющиеся `manual_review` замечания собираются по стабильным
  `code` и сообщению в компактную карточку с числом экземпляров; одно решение
  `approve` или `reject` атомарно охватывает только точный список ID этой
  карточки. Одинаковые пассивные предупреждения также сжимаются в одну строку
  с числом повторений, сохраняя различающиеся поля иерархии. Подсказки сопоставления сохраняют прежние решения `fit` и
  `not_fit` и показывают кандидата, цель и оценку. Выгрузка доступна лишь после
  закрытия обоих наборов решений.
