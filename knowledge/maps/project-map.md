...
- Основной лист XLSX называется `Карточка остатков`. Лист `Сводный отчет`
  ...
  `Все индексы`; каждая содержит восемь категорий. Стоимость суммируется
  формулами всегда, количество — только при полной совместимости непустых
  единиц измерения.
...
- Обе страницы имеют прямой переключатель светлой/темной темы и используют
  ...
  проверки используется один компактный ограниченный по ширине ряд: категория,
  режим учета, `Применить` и `Отклонить`; отдельные дублирующие кнопки смены
  категории запрещены. На узких экранах ряд переносится без overflow.
...
- Реальный артефакт: 3 индекса, 972 чертежа, четыре карточки 2×2 и 64
  сохраненные формулы; LibreOffice-проверка не нашла formula errors.
