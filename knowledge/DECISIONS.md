...
- The values listed in [BUSINESS_RULES §7](../docs/BUSINESS_RULES.md) are blockers, not accepted defaults: M04/M05 exact include sets, M13/M14, suffix/supporting-work semantics, stage/month/whole-period/quantity, 2.7, J/L tolerance, no-compatible-quantity result, conversion/multiple-unit display, versions, formula freshness, feedback policy and AI context/token budget. M02/M06 literals, semantic unit comparison/red marking, matching-unit quantity and all-approved-row cost inclusion are accepted. [[components/document-reconciliation|Component card]]
