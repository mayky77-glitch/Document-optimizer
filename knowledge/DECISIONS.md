...
- The values listed in [BUSINESS_RULES §7](../docs/BUSINESS_RULES.md) are blockers, not accepted defaults: M04/M05 exact include sets, M13/M14, suffix/supporting-work semantics, stage/month/whole-period/quantity, 2.7, J/L tolerance, process-found-only-under-other-unit behavior, conversion/multiple-unit display, versions, formula freshness, feedback policy and AI context/token budget. No-process `0/0` and checkbox review are accepted with the prior unit/cost rules. [[components/document-reconciliation|Component card]]
