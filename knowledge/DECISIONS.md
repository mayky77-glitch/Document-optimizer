...
- The values listed in [BUSINESS_RULES §7](../docs/BUSINESS_RULES.md) are blockers, not accepted defaults: M04/M05 exact include sets, M13/M14, suffix/supporting-work semantics, stage/month/whole-period/quantity, 2.7 scope/editability and warning acknowledgement, J/L tolerance, automatic unit conversion, versions, formula freshness, feedback policy and AI context/token budget. The 2.7 formula/status, no-process `0/0`, checkbox review and alternative-unit grouping are accepted. [[components/document-reconciliation|Component card]]
