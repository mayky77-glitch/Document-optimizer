...
def verify_temp_package(
    ...
    try:
        with zipfile.ZipFile(source_path) as source, zipfile.ZipFile(temp_path) as output:
            ...
            if source_names != tuple(output.namelist()) or output.testzip() is not None:
                raise ExcelWriterIntegrityError(
            ...
            for name in source_names:
                ...
                if name not in changes_by_part and original != updated:
                    raise ExcelWriterIntegrityError("PRESERVATION_CHECK_FAILED", name)
                if name in changes_by_part:
                    expected = original
                    for coordinate, decimal_text in changes_by_part[name]:
