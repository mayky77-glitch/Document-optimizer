...
def manual_review_groups(
    ...
    *,
    ...
    for (code, message, title), items in sorted(grouped.items()):
        ...
        if include_ids:
            record["discrepancy_ids"] = sorted(
                _text(item.get("discrepancy_id"), 200) for item in items
            )
...
def suggestion_review_groups(
    ...
    for item in suggestions:
        suggestion_id, target_ref = (
            _text(item.get("suggestion_id"), 200),
            _text(item.get("target_ref"), 200),
        )
        if (
            item.get("requires_manual_review") is True
            and suggestion_id
            and target_ref
            and suggestion_id not in decided
        ):
...
def issue_contexts(artifacts: Mapping[str, object]) -> dict[str, dict[str, object]]:
    sources = {
        _text(getattr(row, "source_row_id", ""), 200): row
        for row in tuple(getattr(artifacts.get("normalized"), "rows", ()) or ())
    }
    ...
    calculations = {
        _text(getattr(item, "calculation_id", ""), 200): item
        for item in tuple(artifacts.get("calculations", ()) or ())
    }
    report = artifacts.get("quality_report")
    issues = (
        *tuple(getattr(report, "issues", ()) or ()),
        *tuple(artifacts.get("hierarchy_issues", ()) or ()),
    )
    result = {}
    for issue in issues:
        match = match_ids.get(_text(getattr(issue, "match_result_id", ""), 200)) or target_ids.get(
            _text(getattr(issue, "target_row_id", ""), 200)
        )
        calculation = calculations.get(_text(getattr(issue, "calculation_id", ""), 200))
        rows = [
            sources[row_id]
            for row_id in tuple(getattr(issue, "source_row_ids", ()) or ())
            if row_id in sources
        ]
        result[_text(getattr(issue, "issue_id", ""), 200)] = context_record(
            rows, match, calculation
        )
    return result
...
def suggestion_contexts(
    ...
    matches = {
        _text(getattr(item, "result_id", ""), 200): item
        for item in tuple(artifacts.get("matches", ()) or ())
    }
    calculations = {
        _text(getattr(item, "match_result_id", ""), 200): item
        for item in tuple(artifacts.get("calculations", ()) or ())
    }
    result = {}
    for suggestion in tuple(artifacts.get("stage_relation_suggestions", ()) or ()):
        ...
        for candidate in tuple(getattr(suggestion, "candidates", ()) or ()):
            ...
            result[target_id, source_id] = context_record(
                [sources[source_id]] if source_id in sources else [],
                matches.get(target_id),
                calculations.get(target_id),
            )
    return result


def context_record(
    source_rows: Sequence[object], match: object, calculation: object
) -> dict[str, object]:
    source, target = (
        (source_rows[0] if source_rows else None),
        (getattr(match, "target_row", None) or getattr(calculation, "target_row", None)),
    )
    values = {
        "work_name": _text(
        ...
        "proposed_match": _text(
            getattr(target, "stage", None) or getattr(target, "work_name", None)
        ),
    }
    ...
    candidate = getattr(match, "selected_candidate", None) or (
    ...
    quantity = safe_number(
        first_present(
            ...
            getattr(source_value, "period_quantity", None),
        )
    )
    ...
    source_cost = safe_number(
        first_present(
            getattr(source_value, "total_cost", None), getattr(source_value, "period_cost", None)
        )
    )
...
def public_context(value: object) -> dict[str, object]:
    ...
    for key in allowed:
        item = (
            safe_number(value.get(key))
            if key in {"aggregate_cost", "confidence", "quantity", "cost"}
            else _text(value.get(key))
        )
...
def bounded_members(items: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    return [
        {
            "title": _text(public_context(item.get("context")).get("work_name"))
            or "Позиция без наименования",
            "context": public_context(item.get("context")),
        }
        for item in items
    ]
...
def group_context(items: Sequence[Mapping[str, object]]) -> dict[str, object]:
    ...
    costs = list(
        dict.fromkeys(item["aggregate_cost"] for item in contexts if "aggregate_cost" in item)
    )
    if costs:
